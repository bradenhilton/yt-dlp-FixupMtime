...
This postprocessor does not support all possible files output by yt-dlp, namely `*.PART*` and `*.dump` files.
...
```text
--use-postprocessor FixupMtime[:;[mtime_key=<key>][format=<format>]]
```
...
The default value for `format` is `%Y%m%d`. The postprocessor will also attempt to guess the format with yt-dlp's internal list of formats.
...
```text
--use-postprocessor FixupMtime:mtime_key=meta_date;format=%Y-%m-%dT%H.%M.%S
```
